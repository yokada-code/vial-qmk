...
static const key_override_t *grv_override_us_win = {wo_shift(KC_GRV, LALT(KC_GRV))};
static const key_override_t *grv_override_us_mac = {wo_shift(KC_GRV, LCTL(KC_SPC))};
...
void register_jp_key_on_us_os_overrides(void) {
    ...
    for (int idx = 0; idx < sizeof(jp_key_on_us_os_overrides) / sizeof(jp_key_on_us_os_overrides[0]); idx++) {
        register_bmp_key_override(jp_key_on_us_os_overrides[idx]);
    }

    if (keymap_config.swap_lalt_lgui) {
        register_bmp_key_override(grv_override_us_mac);
    } else {
        register_bmp_key_override(grv_override_us_win);
    }
}
